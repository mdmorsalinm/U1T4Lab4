    ...
    public void playRound() {
        ...
        while (!gameOver) {
            ...
            if (correct) {
                ...
                incorrect = 0;
                player1.resetIncorrect();
                player2.resetIncorrect();
                player3.resetIncorrect();
            } else {
                System.out.println("INCORRECT!");
                incorrect++;
                currentPlayer.setIncorrect();
                swapPlayers();
            }
            if (incorrect == 2) {
                gameOver = true;
...
// ------------ PRIVATE HELPER METHODS (internal use only) ------------
    ...
    private void determineWinner() {
        if (!player1.getIncorrect()) {
            winner = player1;
            player1.incrementWins();
            player2.resetWins();
            player3.resetWins();
        } else if (!player2.getIncorrect()) {
            winner = player2;
    ...
    private void doubleWins() {
        if (player1.getWins() >= 2) {
            player1.bonusScore();
        } else if (player2.getWins() >= 2) {
            player2.bonusScore();
        } else if (player3.getWins() >= 2) {
            player3.bonusScore();
        }
    }
}
